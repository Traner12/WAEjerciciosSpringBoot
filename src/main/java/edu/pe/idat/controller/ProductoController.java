...
public class ProductoController {
	...
	public String registrarProducto(
		...
		VentaProducto objvenVentaProducto = new VentaProducto(
				objproducto.getCodProducto(),
				objproducto.getNomProducto(),
				ventaProductoForm.getCantidadTotal(),
				preciototal);
		List<VentaProducto> nuevaListaVentaProducto = productoService.agregarProducto(objvenVentaProducto, lstVentaProducto);
	...
	public String listarVentaProductos(Model model, final HttpSession session) {
		List<VentaProducto> lstventaproducto = (List<VentaProducto>)session.getAttribute("lstproducto");
		if(CollectionUtils.isEmpty(lstventaproducto)) {
			lstventaproducto = new ArrayList<VentaProducto>();
		}
